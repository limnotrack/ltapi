...
[![Codecov test
coverage](https://codecov.io/gh/limnotrack/ltapi/graph/badge.svg)](https://app.codecov.io/gh/limnotrack/ltapi)
<!-- badges: end -->
...
`lt_filter()` supports standard R comparison operators and translates
them into PostgREST query expressions:
...
session, so repeated calls to `lt_tables()`, `lt_schema()`, or
`lt_fetch()` do not re-download the schema. To force a fresh fetch
(e.g. after a database update):
